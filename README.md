...
**PHASE 4 — Moderation & Publishing**. PHASE 5 не начата.
Полная спецификация: [PROJECT_SPEC.md](PROJECT_SPEC.md). Перед каждой новой
фазой сначала прочитайте этот файл.
Результаты реализации, проверок и оставшиеся настройки: [PHASE4_REPORT.md](PHASE4_REPORT.md).
...
- Prisma 7, PostgreSQL, единый ленивый серверный клиент, четыре SQL-миграции.
...
- Демоматериалы только на главной, в presentation layer. Публичные страницы
  статей и социальные функции ещё не реализованы.
...
старую публикацию. На PHASE 4 «Опубликовано» означает корректное состояние БД:
публичные страницы/лента появятся в PHASE 5. Notifications ещё не создаются.
...
Только после отдельной команды: PHASE 5 — публичная лента, страницы статей,
профили, категории и теги, поиск. Публичный контент читается строго из publishedRevision.
