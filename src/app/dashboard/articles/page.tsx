  ...
  return <main id="main-content" tabIndex={-1} className="page-container py-10 sm:py-16"><PrivatePageLifecycle />
    ...
    {!result.articles.length ? <section className="border-y border-border py-20 text-center"><PenLine className="mx-auto mb-6 size-9 text-primary" /><h2 className="font-editorial text-2xl">{result.page === 1 ? "Первая история ещё впереди" : "На этой странице нет статей"}</h2><p className="mx-auto mb-8 mt-4 max-w-md leading-7 text-muted-foreground">{result.page === 1 ? "У вас пока нет статей. Создайте свою первую публикацию в Narra." : "Вернитесь к началу списка."}</p><Button asChild variant="outline"><Link href={result.page === 1 ? "/editor/new" : "/dashboard/articles"}>{result.page === 1 ? "Начать писать" : "К первым статьям"}<ArrowUpRight /></Link></Button></section>
      : <ul className="border-t border-border">{result.articles.map((article) => {
        ...
        return <li key={article.id} className="flex flex-col justify-between gap-5 border-b border-border py-7 lg:flex-row lg:items-center"><div className="min-w-0 flex-1"><h2 className="mb-4 break-words font-editorial text-2xl">{revision?.title.trim() || "Без названия"}</h2><WorkflowStatus status={article.status} publishedRevisionId={article.publishedRevisionId} revision={revision} /><p className="mt-3 text-xs text-muted-foreground">Обновлено <time dateTime={article.updatedAt.toISOString()}>{article.updatedAt.toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })}</time></p></div><div className="flex shrink-0 flex-wrap items-center gap-3 lg:max-w-72"><ArticleWorkflowActions articleId={article.id} status={article.status} publishedRevisionId={article.publishedRevisionId} revision={revision} /><HistoryLink articleId={article.id} /></div></li>;
      })}</ul>}
